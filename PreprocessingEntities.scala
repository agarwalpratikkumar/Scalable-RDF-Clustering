...
object PreprocessingEntities {
  def run(spark: SparkSession, triplesRDD: RDD[org.apache.jena.graph.Triple]) = {
    ...
    //val extractedEntity_SO: RDD[(String,String)] = entityExtraction_SO(parsedTriplesRDD)
    ...
    val extractedEntity_OnlyPredicates: RDD[(String, String)] = entityExtraction_OnlyPredicates(parsedTriplesRDD)
    //extractedEntity_OnlyPredicates.foreach(println)
    LSHoperation.run_dirty(spark, extractedEntity_OnlyPredicates)
  }
  ...
  def get_parsedTriples(triplesRDD: RDD[org.apache.jena.graph.Triple]): RDD[(String, String, Object)] = {
    ...
    .filter(f => ((!f.getPredicate.getURI.contains("birthDate")) && (!f.getPredicate.getURI.contains("deathDate"))))
    .filter(f => (!f.getPredicate.getURI.contains("date")))
    .filter(f => (!f.getPredicate.getURI.contains("birthDate")))
    .filter(f => (!f.getPredicate.getURI.contains("deathDate")))
    .filter(f => (!f.getPredicate.getURI.contains("highlights")))
    ...
    .filter(f=> !f.getPredicate.getURI.contains("#subPropertyOf"))*/
      .map(f => {
        ...
        /*if (f.getObject.getLiteral.contains("^^")) {
          val o = f.getObject.getURI.split("^^").head
          (s, p, o) 
        } */
  ...
  def entityExtraction_SO(parsedTriplesRDD:RDD[(String,String,Object)]):RDD[(String,String)] = {
    return entityExtraction_NoPredicates(parsedTriplesRDD).map(f=>{
      val key = f._1 + ""
      val value = f._1 + " " + f._2
      (key,value)
    })
  ...
  def entityExtraction_SPO(parsedTriplesRDD:RDD[(String, String, Object)]):RDD[(String, String)] = {
    return entityExtraction_AllPredicates(parsedTriplesRDD).map(f=>{
      val key = f._1 + ""
      val value  = f._1 + " "+ f._2
      (key,value)
    })
  ...
  def entityExtraction_OnlyPredicates(parsedTriplesRDD:RDD[(String, String, Object)]):RDD[(String, String)] = {
    return parsedTriplesRDD.map(f => {
      val key = f._1 + "" // Subject is the key
      // val value = f._2 + "  " + f._3 //Predicate + Object is the value
      val value = f._2 + ""
